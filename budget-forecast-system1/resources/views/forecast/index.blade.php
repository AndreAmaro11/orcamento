<x-app-layout>
        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    {{-- Filtros --}}
                    <form method="GET" action="{{ route("forecast.index") }}" class="mb-6 flex flex-wrap items-end space-x-0 sm:space-x-4 space-y-4 sm:space-y-0">
                        {{-- Filtro Ano --}}
                    ...
                    @if ($selectedCostCenterId)
                        <div class="overflow-x-auto" x-data="forecastTable({{ $selectedYear }}, {{ $selectedCostCenterId }})">
                                    ...
                                    @forelse ($accounts as $account)
                                        ...
                                        <tr>
                                            ...
                                            @for ($month = 1; $month <= 12; $month++)
                                                @php 
                                                    $value = $forecastData[$account->id][$month] ?? null;
                                                    $rowTotal += $value ?? 0;
                                                    // Determine if the cell is editable for forecast
                                                    $isEditable = $canEdit && ($selectedYear > $currentYear || ($selectedYear == $currentYear && $month >= $currentMonth));
                                                    $cellId = "forecast-".$selectedYear."-".$month."-".$selectedCostCenterId."-".$account->id;
                                                @endphp
                                                <td class="px-1 py-1 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 text-center relative group {{ !$isEditable ? "bg-gray-100 dark:bg-gray-700" : "" }}">
                                                    @if ($isEditable)
                                                    ...
                                                    @endif
                                                    {{-- Comment Button --}}
                                                    <button @click="openComments({{ $month }}, {{ $account->id }}, "forecast")" 
                                    ...
                                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                        <div class="sm:flex sm:items-start">
                                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                                ...
                                                <div class="mt-4">
                                                    <label for="new_comment_forecast" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Novo Comentário</label>
                                                    <textarea x-model="newCommentText" id="new_comment_forecast" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"></textarea>
                    ...
                    @else
                        <p class="text-center text-gray-500 dark:text-gray-400">Por favor, selecione um centro de custo para visualizar a previsão.</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function forecastTable(selectedYear, selectedCostCenterId) {
            return {
                message: "",
                messageType: "",
                ...
                updateValue(month, accountId, value) {
                    ...
                    .then(({ status, body }) => {
                        ...
                        if (status === 200) {
                            ...
                            // document.getElementById(`forecast-${this.selectedYear}-${month}-${this.selectedCostCenterId}-${accountId}`).value = ... // Need original value
                        }
                ...
                openComments(month, accountId, entryType) {
                    ...
                    this.commentContext.accountCode = document.querySelector(`#forecast-${this.selectedYear}-${month}-${this.selectedCostCenterId}-${accountId}`)?.closest("tr")?.querySelector("td:first-child")?.textContent || "Conta"; 
