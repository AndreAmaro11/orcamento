                    ...
                    <form method="POST" action="{{ route("users.update", $user) }}">
                        ...
                        <div class="mt-4">
                            <x-input-label for="cost_centers" :value="__("Centros de Custo (Opcional - Apenas para Editores/Visualizadores)")" />
                             <select multiple id="cost_centers" name="cost_centers[]" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm h-40">
                                @foreach ($costCenters as $costCenter)
                                    <option value="{{ $costCenter->id }}" {{ $user->costCenters->contains($costCenter->id) ? "selected" : "" }}>
